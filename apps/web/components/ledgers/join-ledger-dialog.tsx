...
export function JoinLedgerDialog({ open, onClose }: Props) {
  const router = useRouter();
  const { join } = useLedgerStore();
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    const lid = id.trim();
    if (!lid) return setError("Ledger ID is required.");
    if (!username) return setError("Log in first.");
    setSubmitting(true);
    const err = await join(lid, username, displayName);
    setSubmitting(false);
    if (err) return setError(err);
    setId(""); setError("");
    onClose();
    router.push(`/team/${lid}`);
  }
    ...
    <div style={overlay}>
      <div style={card}>
        ...
        <form onSubmit={handleSubmit}>
          <label style={lbl}>Ledger ID</label>
          <input
            style={inp}
            placeholder="Paste the Ledger ID here"
            value={id}
          ...
          {username && (
            <p style={{ fontSize: "0.8rem", color: "#6b7280", margin: "6px 0 0" }}>
              Joining as <strong style={{ color: "#4a7af5" }}>{displayName || username}</strong>
            </p>
          )}
